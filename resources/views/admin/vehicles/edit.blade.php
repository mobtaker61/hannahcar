    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('admin.vehicles.show', $vehicle) }}" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">
                    {{ __('View') }}
                </a>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form action="{{ route('admin.vehicles.update', $vehicle) }}" method="POST" enctype="multipart/form-data" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="price" name="price" type="number" step="0.01" class="mt-1 block w-full"
                                         value="{{ old('price', $vehicle->price) }}" min="0" required />
                        ...
                        <div>
                            ...
                            <select id="currency" name="currency" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="AED" {{ old('currency', $vehicle->currency) == 'AED' ? 'selected' : '' }}>AED</option>
                                ...
                                <option value="IRR" {{ old('currency', $vehicle->currency) == 'IRR' ? 'selected' : '' }}>IRR</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="new" {{ old('status', $vehicle->status) == 'new' ? 'selected' : '' }}>{{ __('New') }}</option>
                                <option value="used" {{ old('status', $vehicle->status) == 'used' ? 'selected' : '' }}>{{ __('Used') }}</option>
                                <option value="export" {{ old('status', $vehicle->status) == 'export' ? 'selected' : '' }}>{{ __('Export') }}</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="publish_status" name="publish_status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="draft" {{ old('publish_status', $vehicle->publish_status) == 'draft' ? 'selected' : '' }}>{{ __('Draft') }}</option>
                                <option value="published" {{ old('publish_status', $vehicle->publish_status) == 'published' ? 'selected' : '' }}>{{ __('Published') }}</option>
                                <option value="archived" {{ old('publish_status', $vehicle->publish_status) == 'archived' ? 'selected' : '' }}>{{ __('Archived') }}</option>
                            </select>
                        ...
                        <div class="flex items-center">
                            <input id="is_featured" name="is_featured" type="checkbox" value="1"
                                   {{ old('is_featured', $vehicle->is_featured) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                        ...
                        <div class="flex items-center">
                            <input id="is_available" name="is_available" type="checkbox" value="1"
                                   {{ old('is_available', $vehicle->is_available) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                        ...
                        <div>
                            ...
                            <select id="regional_spec_id" name="regional_spec_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($regionalSpecs as $spec)
                                    <option value="{{ $spec->id }}" {{ old('regional_spec_id', $vehicle->regional_spec_id) == $spec->id ? 'selected' : '' }}>
                                        {{ $spec->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="body_type_id" name="body_type_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($bodyTypes as $type)
                                    <option value="{{ $type->id }}" {{ old('body_type_id', $vehicle->body_type_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="seats_count_id" name="seats_count_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($seatsCounts as $seats)
                                    <option value="{{ $seats->id }}" {{ old('seats_count_id', $vehicle->seats_count_id) == $seats->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="engine_capacity_range_id" name="engine_capacity_range_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($engineCapacityRanges as $engine)
                                    <option value="{{ $engine->id }}" {{ old('engine_capacity_range_id', $vehicle->engine_capacity_range_id) == $engine->id ? 'selected' : '' }}>
                                        {{ $engine->display_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="horsepower_range_id" name="horsepower_range_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($horsepowerRanges as $hp)
                                    <option value="{{ $hp->id }}" {{ old('horsepower_range_id', $vehicle->horsepower_range_id) == $hp->id ? 'selected' : '' }}>
                                        {{ $hp->display_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="cylinders_count_id" name="cylinders_count_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($cylindersCounts as $cylinders)
                                    <option value="{{ $cylinders->id }}" {{ old('cylinders_count_id', $vehicle->cylinders_count_id) == $cylinders->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="steering_side_id" name="steering_side_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($steeringSides as $steering)
                                    <option value="{{ $steering->id }}" {{ old('steering_side_id', $vehicle->steering_side_id) == $steering->id ? 'selected' : '' }}>
                                        {{ $steering->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="exterior_color_id" name="exterior_color_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($exteriorColors as $color)
                                    <option value="{{ $color->id }}" {{ old('exterior_color_id', $vehicle->exterior_color_id) == $color->id ? 'selected' : '' }}>
                                        {{ $color->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="interior_color_id" name="interior_color_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach($interiorColors as $color)
                                    <option value="{{ $color->id }}" {{ old('interior_color_id', $vehicle->interior_color_id) == $color->id ? 'selected' : '' }}>
                                        {{ $color->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="vin_number" name="vin_number" type="text" class="mt-1 block w-full"
                                         value="{{ old('vin_number', $vehicle->vin_number) }}" maxlength="17" />
                        ...
                        <div>
                            ...
                            @if($vehicle->featured_image)
                                <div class="mt-2 mb-2">
                                    <img src="{{ Storage::url($vehicle->featured_image) }}" alt="Current featured image" class="w-32 h-32 object-cover rounded">
                                </div>
                            @endif
                            ...
                            @if($vehicle->gallery && $vehicle->gallery->count() > 0)
                                <div class="mt-2 mb-2 grid grid-cols-4 gap-2">
                                    @foreach($vehicle->gallery as $image)
                                        <img src="{{ Storage::url($image->image_path) }}" alt="Gallery image" class="w-16 h-16 object-cover rounded">
                                    @endforeach
                                </div>
                            @endif
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="features" name="features" type="text" class="mt-1 block w-full"
                                         value="{{ old('features', is_array($vehicle->features) ? implode(', ', $vehicle->features) : $vehicle->features) }}" placeholder="Feature 1, Feature 2, Feature 3" />
