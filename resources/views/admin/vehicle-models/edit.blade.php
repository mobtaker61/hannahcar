            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <div class="p-6">
                    <div class="flex items-center mb-6">
                        ...
                        <h3 class="text-lg font-medium text-gray-900">{{ __('Edit Vehicle Model') }}</h3>
                    </div>

                    <form action="{{ route('admin.vehicle-models.update', $vehicleModel) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="relative">
                                ...
                                <input type="text"
                                       ...
                                       value="{{ $vehicleModel->brand->name ?? '' }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                       autocomplete="off">
                                <input type="hidden" name="brand_id" id="brand_id" value="{{ $vehicleModel->brand_id }}" required>
                            ...
                            <div>
                                ...
                                <input type="number"
                                       name="sort_order"
                                       id="sort_order"
                                       value="{{ old('sort_order', $vehicleModel->sort_order) }}"
                                       min="0"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                @error('sort_order')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="description"
                                       id="description"
                                       rows="3"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $vehicleModel->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
