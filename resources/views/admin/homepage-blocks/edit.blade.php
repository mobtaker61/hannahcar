<x-admin-layout>
    <x-slot name="header">
        ویرایش بلوک: {{ $homepageBlock->translations->first()?->title ?? 'بدون عنوان' }}
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.homepage-blocks.update', $homepageBlock) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="type" id="type"
                                        ...
                                        <option value="featured" {{ old('type', $homepageBlock->type) === 'featured' ? 'selected' : '' }}>ویژه</option>
                                        <option value="service" {{ old('type', $homepageBlock->type) === 'service' ? 'selected' : '' }}>خدمات</option>
                                        <option value="testimonial" {{ old('type', $homepageBlock->type) === 'testimonial' ? 'selected' : '' }}>نظرات</option>
                                        <option value="news" {{ old('type', $homepageBlock->type) === 'news' ? 'selected' : '' }}>اخبار</option>
                                        <option value="stats" {{ old('type', $homepageBlock->type) === 'stats' ? 'selected' : '' }}>آمار</option>
                                    </select>
                                    @error('type')
                                ...
                                <div>
                                    ...
                                    <input type="text" name="image" id="image" value="{{ old('image', $homepageBlock->image) }}"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="button_url" id="button_url" value="{{ old('button_url', $homepageBlock->button_url) }}"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="icon" id="icon" value="{{ old('icon', $homepageBlock->icon) }}"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                ...
                                <div>
                                    ...
                                    <input type="color" name="background_color" id="background_color" value="{{ old('background_color', $homepageBlock->background_color) }}"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $homepageBlock->sort_order) }}"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                           placeholder="0">
                                    @error('sort_order')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div class="mt-6">
                                <div class="flex items-center">
                                    <input type="checkbox" name="is_active" id="is_active" value="1"
                                           {{ old('is_active', $homepageBlock->is_active) ? 'checked' : '' }}
                        ...
                        <div class="mb-8">
                            ...
                            @foreach($languages as $language)
                                @php
                                    $translation = $homepageBlock->translations->where('language_id', $language->id)->first();
                                @endphp
                                <div class="border border-gray-200 rounded-lg p-6 mb-6">
                                    ...
                                    <div class="mt-6">
                                        ...
                                        <textarea name="translations[{{ $language->id }}][description]" rows="3"
                                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                  placeholder="توضیحات بلوک">{{ old("translations.{$language->id}.description", $translation?->description) }}</textarea>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="translations[{{ $language->id }}][meta_data]"
                                                   value="{{ old("translations.{$language->id}.meta_data", $translation?->meta_data) }}"
                                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
